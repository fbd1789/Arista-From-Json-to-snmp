...
var UnsupportedOperationErr error = errors.New("Unsupported operation")
...
func NewOid(s string) (oid *Oid, err error) {
	...
	// RFC2578 Section 3.5
	if len(subids) > 128 {
		return nil, &InvalidOidError{s, "The sub-identifiers in an OID is up to 128"}
	}
	...
	for i, v := range subids {
		...
		if err != nil || o[i] < 0 || int64(o[i]) > math.MaxUint32 {
			return nil, &InvalidOidError{s, fmt.Sprintf("The sub-identifiers is range %d..%d", 0, int64(math.MaxUint32))}
		}
	}

	if len(o) > 0 && o[0] > 2 {
		return nil, &InvalidOidError{s, "The first sub-identifier is range 0..2"}
	}

	// ISO/IEC 8825 Section 8.19.4
	if len(o) < 2 {
		return nil, &InvalidOidError{s, "The first and second sub-identifier is required"}
	}

	if o[0] < 2 && o[1] >= 40 {
		return nil, &InvalidOidError{s, "The second sub-identifier is range 0..39"}
	}
